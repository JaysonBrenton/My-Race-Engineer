...
test('absUrl falls back to NEXT_PUBLIC_APP_URL when APP_URL is missing', { concurrency: false }, async () => {
  const { absUrl, __resetAppUrlCacheForTests } = await import('../src/lib/seo');
  delete process.env.APP_URL;
...
test('absUrl falls back to default localhost origin when no URLs are configured', { concurrency: false }, async () => {
  const { absUrl, __resetAppUrlCacheForTests } = await import('../src/lib/seo');
  delete process.env.APP_URL;
  delete process.env.NEXT_PUBLIC_APP_URL;
...
test('robots.txt references the sitemap and disallows sensitive paths', { concurrency: false }, async () => {
  const { __resetAppUrlCacheForTests } = await import('../src/lib/seo');
  __resetAppUrlCacheForTests();
